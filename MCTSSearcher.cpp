...
#else
        if (untried & (Hex<HEX_SIZE>::uintsize_t(1) << a)) {
            return a;
        }
#endif
    }
...
#pragma HLS PIPELINE II=1
            bool isLegal = false;
#if MCTS_HLS_TYPES
            isLegal = legalActionMap[i];
#else
            isLegal = (legalActionMap & (Hex<HEX_SIZE>::uintsize_t(1UL) << i)) != 0;
#endif
            if (isLegal) {
...
int search(const Hex<HEX_SIZE> boardState, const bool isRED) {
